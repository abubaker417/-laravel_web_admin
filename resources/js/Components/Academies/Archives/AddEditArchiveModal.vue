<template>
    ...
    <Modal maxWidth="lg" :id="id">
            ...
            <div class="modal-header">
                <h5 class="modal-title ms-2 text-primary fs-2 fw-bold" id="exampleModalLabel">{{ modalData ? __('update'):__('create') }} {{__('archive')}}</h5>
            ...
            <div class="modal-body">
                <div class="row">
                    <ul class="nav modal-tabs nav-tabs gap-3 px-3 border-bottom border-primary border-2 border-opacity-25 my-2" id="myTab" role="tablist">
                        <li v-for="(lang, index) in $page.props.translation_languages" :key="lang.id" class="nav-item" role="presentation">
                            <button :class="{'active': index == 0}" class="nav-link mr-1 fw-bold" :id="`archive-nav-${lang.code}-tab`" data-bs-toggle="tab" :data-bs-target="`#archive-nav-${lang.code}`" type="button" role="tab" :aria-controls="`archive-nav-${lang.code}`" aria-selected="true">{{ lang.name }}</button>
                        </li>
                    </ul>
                    <div v-for="(lang, secondIndex) in $page.props.translation_languages" :key="lang.id" class="tab-content mt-1" id="myTabContent">
                        <div :class="{'show active': secondIndex == 0}" class="tab-pane fade mb-2" :id="`archive-nav-${lang.code}`" role="tabpanel" :aria-labelledby="`archive-nav-${lang.code}-tab`">
                        <div class="col-md-12">
                    ...
                    <div class="col-md-6">
                        <div class="form-group mb-3">
                            ...
                            <select v-model="form.archive_category_id" class="form-select rounded-3"
                                aria-label="category">
                                <option value="">{{ __('select') }}</option>
                                <option v-for="cat in this.archive_categories" :key="cat.id" :value="cat.id">{{
                                    cat.name
                                }}</option>
                            </select>
                        </div>
                ...
                <div>
                    <!-- Cropped image previewer -->
                        <ImageCropperModal :show="showImportModal" id="archiveImageCropModal" :image_url="image_url" @cropImage="cropImage" ></ImageCropperModal>
...
export default defineComponent({
    ...
    data(){
        return {
            ...
            archive_categories: this.$page.props.archive_categories,
    ...
    methods:{
        ...
        create(){
            this.form.post(this.route('academies.academy_archives.store'), {
                preserveState: true,
                preserveScroll: true,
                onSuccess: () => {
        ...
        update(){
            this.form.transform((data) => ({
                    ...data,
                    _method: "PUT",
                })).post(this.route('academies.academy_archives.update',{academy_archive:this.modalData.id}), {
                preserveState: true,
                preserveScroll: true,
                onSuccess: () => {
